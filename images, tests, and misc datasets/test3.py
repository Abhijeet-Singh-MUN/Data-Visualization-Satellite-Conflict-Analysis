...
def load_midip(file_path='MIDIP 5.0.csv'):
    ...
    220: 'Western Europe',      # France (matches SATCAT 'F' code)
    255: 'Western Europe',      # Germany (matches SATCAT 'D' code)
...
def update_figure(selected_year):
    ...
    for i, country in enumerate(country_order):
        ...
        if not pd.isna(hostility_level):
            fig.add_trace(go.Bar(
                ...
                marker_color='rgba(200, 0, 0, 0.7)', showlegend=False, 
            ...
            for i, country in enumerate(country_order):
                ...
                if not country_subset.empty:
                    ...
                    fig.add_trace(go.Scatter(
                        ...
                        mode='markers', marker_symbol=symbol,
                        marker=dict(
                            size=10, color=country_colors[country],
                            line=dict(width=1, color='DarkSlateGrey')
                        ),
                        name=purpose, showlegend=(i==0 and orbit=='LEO'),
                        hovertemplate=(
    ...
    fig.update_layout(
        ...
        yaxis2=dict(
            ...
            range=[0, 5],  # Fixed scale for comparison
            side='right',
            showgrid=False
        ),
        yaxis={'tickvals': [1,2,3], 'ticktext': ['LEO', 'MEO', 'GEO'],
              'title': 'Orbital Class', 'range': [0.5, 3.5]},
        template="plotly_white",
        margin=dict(b=150),
        annotations=[dict(
            x=0.5, y=1.07, xref='paper', yref='paper',
            text="<b>○ Civil ◇ Defense □ Business △ Comms ⬡ Nav ✖ Science ★ Academic</b>",
            showarrow=False, font=dict(size=16),
            bgcolor="rgba(255,255,255,0.9)", bordercolor="#2a3f5f"
        )]
    )
    return fig
